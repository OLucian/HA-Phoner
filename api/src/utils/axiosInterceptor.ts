...
function createAuthRefreshInterceptor(axios: any, refreshTokenCall: any, options = {}) {
  const id = axios.interceptors.response.use((res: any) => res, (error: any) => {
      // Reject promise if the error status is not in options.ports or defaults.ports
      const { statusCodes } = defaults;
      ...
      const requestQueueInterceptorId = axios.interceptors
          .request
          .use((request: any) => refreshCall.then(() => request));
